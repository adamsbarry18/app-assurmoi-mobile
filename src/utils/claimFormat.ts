...
/** Options de type pour le menu d’import (ex. assuré vs collaborateur). */
export function apiDocumentUploadTypesForRole(
  role: string | undefined
): { value: string; label: string }[] {
  if (role === 'INSURED') {
    return API_DOCUMENT_UPLOAD_TYPES.filter((o) => INSURED_UPLOAD_VALUES.has(o.value))
  }
  return API_DOCUMENT_UPLOAD_TYPES
}
