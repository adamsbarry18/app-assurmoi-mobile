...
import { consumeScreenFeedback } from '@/utils/screenFeedback'
import {
  activateUserApi,
  cancelInvitation,
  deactivateUserApi,
...
import { canFullUserAdmin, canListUsers, roleLabel } from '@/utils/roleAccess'
import type { ListUserRow, ListInvitationRow, AuthUser } from '@/auth/types'
...
export type UserDirectoryScreenProps = {
  userScope: 'insured' | 'staff'
  screenTitle: string
  /** Liste équipe : invitation par e-mail */
  showInviteFab?: boolean
  /** Liste assurés : création fiche assuré */
  showNewInsuredFab?: boolean
}
...
export function UserDirectoryScreen({
  ...
  }, [navigation, logout, router, screenTitle])
  ...
  const onResendInsuredWelcome = (id: number) => {
    setMenuKey(null)
    setActionBusy(true)
    void (async () => {
      try {
        await resendInsuredWelcome(id)
        setListSnack('E-mail de premier accès renvoyé.')
        await fetchUsers('list')
      } catch (e) {
        const m = e instanceof ApiRequestError ? e.message : 'Action impossible'
        Alert.alert('Erreur', m)
  ...
  if (!user || !canListUsers(user.role)) {
    return null
  }
                    ...
                    const rowContent = (
                      ...
                      >
                        ...
                        >
                          ...
                          ) : isUser(item) && item.password_pending ? (
                            <View
                              style={{
                                backgroundColor: 'rgba(191, 94, 0, 0.15)',
                                paddingHorizontal: 6,
                                paddingVertical: 3,
                                borderRadius: 6,
                                alignSelf: 'flex-start'
                        ...
                        >
                          ...
                          ((isInvitation(item) && item.status === 'pending') ||
                            (isUser(item) &&
                              user.id !== item.id &&
                              (canFullUserAdmin(user.role) ||
                                (item.role === 'INSURED' && item.password_pending)))) ? (
                            <Menu
                            ...
                            >
                              ...
                              {isUser(item) &&
                              item.role === 'INSURED' &&
                              item.password_pending &&
                              user.id !== item.id ? (
                                <Menu.Item
                                  onPress={() => onResendInsuredWelcome(item.id)}
                                  title="Renvoyer e-mail 1er accès"
                                />
                              ) : null}
                              {isInvitation(item) && item.status === 'pending' ? (
                                <>
